...
function devOsmProxy(): Plugin {
  return {
    ...
    configureServer(server) {
      // Local stub: production uses Vercel x-vercel-ip-* headers.
      server.middlewares.use('/api/geo-fallback', (req, res) => {
      ...
      server.middlewares.use('/api/osm-discovery', async (req, res) => {
        ...
        if (
          ...
          !Number.isFinite(radiusMeters) || radiusMeters <= 0 || radiusMeters > 50000 ||
          !clauses.length
        ) {
          res.statusCode = 400;
          res.setHeader('Content-Type', 'application/json');
          res.end(JSON.stringify({ error: 'Invalid discovery request.' }));
          return;
        }
          ...
          try {
            const response = await fetch(endpoint, {
              ...
              headers: {
                ...
                'User-Agent': 'VYBE local discovery proxy/1.0',
              },
        ...
        res.statusCode = 503;
        res.setHeader('Content-Type', 'application/json');
        res.end(JSON.stringify({ error: 'OpenStreetMap is temporarily unavailable. Showing Google results instead.' }));
      });
    },
  };
}
