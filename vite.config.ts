...
function devOsmProxy(): Plugin {
  return {
    ...
    configureServer(server) {
      server.middlewares.use('/api/osm-discovery', async (req, res, next) => {
        ...
        const clauses = Array.isArray(body.clauses) ? body.clauses.filter((v): v is string => typeof v === 'string' && (ALLOWED_CLAUSE.test(v) || SAFE_NAME_CLAUSE.test(v))).slice(0, 6) : [];
        if (!Number.isFinite(lat) || lat < -90 || lat > 90 || !Number.isFinite(lng) || lng < -180 || lng > 180 || !Number.isFinite(radiusMeters) || radiusMeters <= 0 || radiusMeters > 50000 || !clauses.length) {
          res.statusCode = 400; res.setHeader('Content-Type', 'application/json'); res.end(JSON.stringify({ error: 'Invalid discovery request.' })); return;
        }
        ...
        for (const endpoint of OSM_ENDPOINTS) {
          ...
          try {
            const response = await fetch(endpoint, { method: 'POST', headers: { 'Content-Type': 'application/x-www-form-urlencoded; charset=UTF-8', 'User-Agent': 'VYBE local discovery proxy/1.0' }, body: new URLSearchParams({ data: query }), signal: controller.signal });
            if (response.ok) {
              res.statusCode = 200; res.setHeader('Content-Type', 'application/json'); res.end(await response.text()); return;
            }
          } catch {
            // Try the next mirror.
          } finally {
