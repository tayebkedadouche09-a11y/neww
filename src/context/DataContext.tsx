...
interface DataContextType {
  ...
  discover: (overrideFilters?: FilterState) => void;
...
const INITIAL_COLLECTIONS: Collection[] = [
  {
    id: 'col-1', userId: 'u-1', name: 'Weekend Hype & Sunsets',
    description: 'Golden hour rooftops, neon arcades & late night street tacos', emoji: '🌆', color: '#CCFF00', isPublic: true,
    placeIds: ['place-1', 'place-2', 'place-15'], createdAt: '2025-01-10T10:00:00Z', updatedAt: '2025-01-15T10:00:00Z'
  },
  {
    id: 'col-2', userId: 'u-1', name: 'Lo-Fi Chill & Matcha Nooks',
    description: 'Best spots to read, draw or listen to ambient vinyl', emoji: '🍵', color: '#00F0FF', isPublic: true,
    placeIds: ['place-3', 'place-12', 'place-16'], createdAt: '2025-01-12T14:00:00Z', updatedAt: '2025-01-18T16:00:00Z'
  },
  {
    id: 'col-3', userId: 'u-1', name: 'Date Night Magic',
    description: 'Aesthetic speakeasies, pottery workshops & LED glowing kayaks', emoji: '✨', color: '#FF007F', isPublic: false,
    placeIds: ['place-4', 'place-5', 'place-11', 'place-18'], createdAt: '2025-01-14T19:00:00Z', updatedAt: '2025-01-20T21:00:00Z'
  }
];

const INITIAL_PLANS: VybePlan[] = [
  {
    id: 'plan-1', userId: 'u-1', title: 'Epic Friday Night Outing', date: 'Tonight', mood: 'party', targetBudgetUsd: 65,
    coverImage: 'https://images.unsplash.com/photo-1517248135467-4c7edcad34c4?auto=format&fit=crop&w=800&q=80', isPublic: true,
...
export const DataProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const [collections, setCollections] = useState<Collection[]>(() => {
    ...
    if (saved) { try { return JSON.parse(saved); } catch (e) { console.error('Failed to parse collections', e); } }
    return INITIAL_COLLECTIONS;
  });
  const [plans, setPlans] = useState<VybePlan[]>(() => {
    const saved = localStorage.getItem(LOCAL_STORAGE_KEYS.plans);
    if (saved) { try { return JSON.parse(saved); } catch (e) { console.error('Failed to parse plans', e); } }
  ...
  const discover = useCallback((overrideFilters?: FilterState) => {
    ...
    if (userLat === undefined || userLng === undefined) {
      console.log('[discovery] No browser location available — discovery waits for location.');
      return;
    }

    setDiscoveryLoading(true);
    setDiscoveryError(null);
    console.log(`[discovery] location=(${userLat.toFixed(5)}, ${userLng.toFixed(5)}) query=${activeFilters.searchQuery || 'broad'}`);
    ...
    discoverPlaces({
      ...
      .then(result => {
        console.log(`[discovery] places.length: ${result.length}`);
        setPlaces(result);
      })
      .catch(err => {
  ...
  useEffect(() => {
    if (!geo.location && !geo.loading && !geo.error) geo.requestLocation();
  }, [geo.location, geo.loading, geo.error, geo.requestLocation]);

  useEffect(() => {
    if (geo.location) discover();
  ...
  const openShareModal = (place?: Place) => { setShareTargetPlace(place || selectedPlace || places[0] || null); setIsShareModalOpen(true); };

  const createPlan = (title: string, mood: MoodType, targetBudgetUsd = 50) => {
    const newPlan: VybePlan = { id: dataMode === 'supabase' ? newUuid() : `plan-${Date.now()}`, userId: currentUser?.id || 'u-1', title, date: 'Upcoming Outing', mood, targetBudgetUsd, isPublic: true, createdAt: new Date().toISOString(), items: [] };
    setPlans(prev => [newPlan, ...prev]); setActivePlan(newPlan);
    if (realUserId) void plansService.create(newPlan).catch(e => console.error('Plan create sync failed', e));
    showToast(`Created new VYBE Plan: "${title}"`, '📋', 'success'); return newPlan;
  };

  const addPlaceToPlan = (planId: string, placeId: string, customTime = '20:00') => {
    const plan = plans.find(p => p.id === planId); const place = places.find(p => p.id === placeId); if (!plan || !place) return;
    if (plan.items.some(item => item.placeId === placeId)) { showToast(`${place.name} is already in this plan!`, 'ℹ️', 'info'); return; }
    ...
    const updatedPlans = plans.map(p => p.id === planId ? { ...p, items: [...p.items, newItem] } : p); setPlans(updatedPlans);
  ...
  const removePlaceFromPlan = (planId: string, planItemId: string) => {
    const updatedPlans = plans.map(p => p.id === planId ? { ...p, items: p.items.filter(item => item.id !== planItemId) } : p); setPlans(updatedPlans);
    if (activePlan?.id === planId) setActivePlan(updatedPlans.find(p => p.id === planId) || null);
    if (realUserId) void plansService.removeItem(planItemId).catch(e => console.error('Plan item remove sync failed', e)); showToast('Removed item from plan', '🗑️', 'info');
  };
  const updatePlanItem = (planId: string, planItemId: string, updates: { startTime?: string; customNote?: string; durationMinutes?: number }) => {
    const updatedPlans = plans.map(p => p.id === planId ? { ...p, items: p.items.map(item => item.id === planItemId ? { ...item, ...updates } : item) } : p); setPlans(updatedPlans);
    if (activePlan?.id === planId) setActivePlan(updatedPlans.find(p => p.id === planId) || null);
    if (realUserId) void plansService.updateItem(planItemId, updates).catch(e => console.error('Plan item update sync failed', e));
  };
  const deletePlan = (planId: string) => {
    setPlans(prev => prev.filter(p => p.id !== planId)); if (activePlan?.id === planId) setActivePlan(plans.find(p => p.id !== planId) || null);
    if (realUserId) void plansService.remove(planId).catch(e => console.error('Plan delete sync failed', e)); showToast('Plan deleted', '🗑️', 'info');
  };

  const createCollection = (name: string, emoji: string, color: string, description = '') => {
    const nowIso = new Date().toISOString();
    const newCol: Collection = { id: dataMode === 'supabase' ? newUuid() : `col-${Date.now()}`, userId: currentUser?.id || 'u-1', name, description, emoji, color, isPublic: true, placeIds: [], createdAt: nowIso, updatedAt: nowIso };
    setCollections(prev => [newCol, ...prev]); if (realUserId) void collectionsService.create(newCol).catch(e => console.error('Collection create sync failed', e)); showToast(`Created collection: ${emoji} ${name}`, '✨', 'success'); return newCol;
  };
  const addPlaceToCollection = (collectionId: string, placeId: string) => {
    const col = collections.find(c => c.id === collectionId); const place = places.find(p => p.id === placeId); if (!col || !place) return;
    if (col.placeIds.includes(placeId)) { showToast(`Already saved in "${col.name}"`, 'ℹ️', 'info'); return; }
    setCollections(prev => prev.map(c => c.id === collectionId ? { ...c, placeIds: [...c.placeIds, placeId], updatedAt: new Date().toISOString() } : c));
    if (realUserId) void collectionsService.addPlace(collectionId, placeId).catch(e => console.error('Collection item sync failed', e)); showToast(`Saved to "${col.name}"!`, col.emoji || '💖', 'success');
  };
  const removePlaceFromCollection = (collectionId: string, placeId: string) => { setCollections(prev => prev.map(c => c.id === collectionId ? { ...c, placeIds: c.placeIds.filter(id => id !== placeId) } : c)); if (realUserId) void collectionsService.removePlace(collectionId, placeId).catch(e => console.error('Collection item remove sync failed', e)); showToast('Removed from collection', '🗑️', 'info'); };
  const deleteCollection = (collectionId: string) => { setCollections(prev => prev.filter(c => c.id !== collectionId)); if (realUserId) void collectionsService.remove(collectionId).catch(e => console.error('Collection delete sync failed', e)); showToast('Collection deleted', '🗑️', 'info'); };

  const addReview = (placeId: string, reviewData: Omit<PlaceReview, 'id' | 'createdAt' | 'likesCount'>) => {
    const target = places.find(p => p.id === placeId); if (!target) return;
    const newReview: PlaceReview = { ...reviewData, id: dataMode === 'supabase' ? newUuid() : `rev-${Date.now()}`, createdAt: 'Just now', likesCount: 0 };
    const newAvgRating = Number(((target.rating * target.reviewCount + reviewData.rating) / (target.reviewCount + 1)).toFixed(1));
    const updatedPlace: Place = { ...target, reviews: [newReview, ...target.reviews], rating: newAvgRating, reviewCount: target.reviewCount + 1 };
    setPlaces(prev => prev.map(p => p.id === placeId ? updatedPlace : p)); setSelectedPlace(cur => cur && cur.id === placeId ? updatedPlace : cur);
    if (realUserId) void reviewsService.create({ placeId, userId: reviewData.userId, rating: reviewData.rating, vibeRating: reviewData.vibeRating, moodTags: reviewData.moodTags, comment: reviewData.comment, id: newReview.id }).catch(e => console.error('Review sync failed', e));
  ...
  const addPlace = (placeData: Omit<Place, 'id' | 'rating' | 'reviewCount' | 'baseVybeScore' | 'reviews'>) => {
    const newPlace: Place = { ...placeData, id: dataMode === 'supabase' ? newUuid() : `place-${Date.now()}`, rating: 4.8, reviewCount: 1, baseVybeScore: 92, reviews: [] };
    setPlaces(prev => [newPlace, ...prev]); if (realUserId) void placesService.create(newPlace).catch(e => console.error('Place create sync failed', e)); showToast(`Added new place: "${newPlace.name}"`, '🎉', 'success'); return newPlace;
  };
  const updatePlace = (placeId: string, updates: Partial<Place>) => { setPlaces(prev => prev.map(p => p.id === placeId ? { ...p, ...updates } : p)); if (selectedPlace?.id === placeId) setSelectedPlace(prev => prev ? { ...prev, ...updates } : null); if (realUserId) void placesService.update(placeId, updates).catch(e => console.error('Place update sync failed', e)); showToast('Place updated successfully!', '✏️', 'success'); };
  const deletePlace = (placeId: string) => { setPlaces(prev => prev.filter(p => p.id !== placeId)); if (selectedPlace?.id === placeId) { setSelectedPlace(null); setIsDetailOpen(false); } if (realUserId) void placesService.remove(placeId).catch(e => console.error('Place delete sync failed', e)); showToast('Place removed from platform', '🗑️', 'info'); };
    ...
    const result = places
      .filter(place => {
        if (effectiveMoods.length > 0) {
          const hasMood = effectiveMoods.includes(place.primaryMood) || place.secondaryMoods.some(m => effectiveMoods.includes(m));
          if (!hasMood) return false;
        }
        if (filters.categories.length > 0 && !filters.categories.includes(place.category)) return false;
        if (filters.priceLevels.length > 0 && !filters.priceLevels.includes(place.priceLevel)) return false;
  ...
  return (
    <DataContext.Provider value={{
      ...
      requestLocationAndDiscover, discover, selectedPlace, setSelectedPlace, isDetailOpen, setIsDetailOpen,
      openPlaceDetail, isReviewModalOpen, setIsReviewModalOpen, isShareModalOpen, setIsShareModalOpen, shareTargetPlace,
      openShareModal, isAuthModalOpen, setIsAuthModalOpen, authModalMode, setAuthModalMode, plans, activePlan, setActivePlan,
      createPlan, addPlaceToPlan, removePlaceFromPlan, updatePlanItem, deletePlan, collections, createCollection,
      addPlaceToCollection, removePlaceFromCollection, deleteCollection, addReview, addPlace, updatePlace, deletePlace,
      filteredPlaces, toasts, showToast, removeToast
    }}>
      {children}
