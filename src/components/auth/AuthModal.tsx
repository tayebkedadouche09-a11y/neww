...
const strongPassword = (value: string) => value.length >= MIN_PASSWORD_LENGTH
  && /[a-z]/.test(value)
  && /[A-Z]/.test(value)
  && /\d/.test(value)
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!strongPassword(password)) {
        showToast('Password must be 12+ characters with uppercase, lowercase, number, and symbol.', '🔒', 'info');
        return;
      }
