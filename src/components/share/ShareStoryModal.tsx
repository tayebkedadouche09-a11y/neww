...
export const ShareStoryModal: React.FC = () => {
  ...
  const handleCopyLink = () => {
    ...
    setIsCopied(true);
    showToast('Copied VYBE link to clipboard!', '📋', 'success');
    setTimeout(() => setIsCopied(false), 2500);
  };

  const handleDownloadCard = () => {
    // Generate Canvas render
      ...
      >
        ...
        <div>
          ...
          <p className="text-xs text-slate-600 dark:text-slate-400">
            Formatted specifically for Instagram Stories, TikTok & group chats.
          </p>
        </div>

        {/* Live Story Preview Canvas Container */}
          ...
          >
            ...
            <div className="space-y-1 relative z-10">
              <div className="flex items-center justify-between">
                ...
                <span className="text-[10px] font-mono px-2 py-0.5 rounded bg-white/10 text-vybe-lime">
                  STORY
                </span>
              </div>
              <p className="text-[10px] font-mono text-slate-400">
                Curated by @{currentUser?.username || 'kaivybes'}
              </p>
            </div>
                ...
                <button
                  ...
                  className={`p-2 rounded-xl text-xs font-bold border transition-all flex items-center gap-2 ${
                    themeGradient === g.id
                      ...
                      : 'bg-slate-100 dark:bg-vybe-dark-surface border-slate-200 dark:border-vybe-dark-border text-slate-700 dark:text-slate-300'
                  }`}
                >
              ...
              {['🔥 MUST TRY', '🌙 10/10 VIBE', '🚀 SQUAD APPROVED', '✨ SECRET SPOT', '🍜 BEST EATS'].map(s => (
                <button
                  ...
                  className={`px-3 py-1.5 rounded-xl text-xs font-bold border transition-all ${
                    selectedSticker === s
                      ? 'bg-vybe-citrus text-white border-vybe-citrus shadow-neon-citrus'
                      : 'bg-slate-100 dark:bg-vybe-dark-surface border-slate-200 dark:border-vybe-dark-border text-slate-700 dark:text-slate-300'
                  }`}
                >
                  {s}
        ...
        <div className="flex items-center justify-between gap-3 pt-4 border-t border-slate-200 dark:border-white/10">
          <button
            ...
            className="flex-1 py-3 px-4 rounded-2xl bg-slate-100 dark:bg-vybe-dark-surface text-slate-900 dark:text-white font-bold text-xs border border-slate-200 dark:border-vybe-dark-border hover:border-vybe-lime flex items-center justify-center gap-2 transition-all"
          >
          ...
          <button
            ...
            className="flex-1 py-3 px-4 rounded-2xl bg-vybe-lime text-black font-display font-extrabold text-xs uppercase tracking-wider shadow-neon-lime hover:scale-105 flex items-center justify-center gap-2 transition-all"
          >
