...
function haversineKm(aLat: number, aLng: number, bLat: number, bLng: number): number { const r = 6371; const dLat = ((bLat - aLat) * Math.PI) / 180; const dLng = ((bLng - aLng) * Math.PI) / 180; const x = Math.sin(dLat / 2) ** 2 + Math.cos((aLat * Math.PI) / 180) * Math.cos((bLat * Math.PI) / 180) * Math.sin(dLng / 2) ** 2; return r * 2 * Math.atan2(Math.sqrt(x), Math.sqrt(1 - x)); }

function inferCategory(tags: Record<string, string>, name: string) {
  const hay = normalize([name, ...Object.values(tags)].join(' '));
  const has = (...words: string[]) => words.some(word => hay.includes(normalize(word)));
  if (has('arcade', 'gaming', 'game room', 'salle de jeux', 'jeux video', 'video games', 'playstation', 'xbox', 'cyber')) return { category: 'gaming' as const, mood: 'gaming' as const, labels: ['Gaming', 'Social', 'Fun'], reasons: ['The place name or tags indicate gaming or play.'] };
  if (has('restaurant', 'resto', 'pizzeria', 'pizza', 'burger', 'tacos', 'grill', 'snack', 'fast food', 'food court')) return { category: 'food' as const, mood: 'hungry' as const, labels: ['Food', 'Group-friendly'], reasons: ['Food-service signals were found in the place metadata.'] };
  if (has('cafe', 'coffee', 'café', 'tea room', 'salon de thé', 'bakery', 'patisserie')) return { category: 'cafe' as const, mood: 'chill' as const, labels: ['Chill', 'Coffee'], reasons: ['Cafe, coffee or tea signals were found.'] };
  if (has('bar', 'pub', 'club', 'nightclub', 'discotheque', 'karaoke', 'lounge')) return { category: 'nightlife' as const, mood: 'party' as const, labels: ['Nightlife', 'Social'], reasons: ['Nightlife keywords or venue types were found.'] };
  if (has('museum', 'gallery', 'theatre', 'theater', 'library', 'mosque', 'mosquée', 'church', 'église', 'cultural', 'heritage')) return { category: 'culture' as const, mood: 'curious' as const, labels: ['Culture', 'Explore'], reasons: ['Cultural, heritage or learning signals were found.'] };
  if (has('park', 'garden', 'beach', 'plage', 'forest', 'forêt', 'nature', 'promenade', 'hiking', 'randonnée', 'zoo', 'aquarium')) return { category: 'outdoors' as const, mood: 'outdoor' as const, labels: ['Outdoor', 'Fresh air'], reasons: ['Outdoor, nature or attraction signals were found.'] };
  if (has('mall', 'centre commercial', 'shopping', 'boutique', 'store', 'shop', 'market', 'marché', 'friperie')) return { category: 'shopping' as const, mood: 'explore' as const, labels: ['Shopping', 'Explore'], reasons: ['Retail or shopping signals were found.'] };
  if (has('gym', 'fitness', 'stadium', 'sports', 'sport', 'pool', 'piscine', 'tennis', 'football')) return { category: 'sports' as const, mood: 'energetic' as const, labels: ['Active', 'Sports'], reasons: ['Sports or fitness signals were found.'] };
  if (has('spa', 'wellness', 'relax', 'yoga', 'massage')) return { category: 'wellness' as const, mood: 'lazy' as const, labels: ['Wellness', 'Relax'], reasons: ['Wellness or relaxation signals were found.'] };
  if (has('hotel', 'hostel', 'guest house', 'motel', 'resort', 'apartment')) return { category: 'stay' as const, mood: 'chill' as const, labels: ['Stay', 'Chill'], reasons: ['Accommodation signals were found.'] };
  if (has('school', 'university', 'bank', 'post office', 'police', 'hospital', 'clinic', 'pharmacy')) return { category: 'services' as const, mood: 'explore' as const, labels: ['Service'], reasons: ['A service or essential facility signal was found.'] };
  return { category: 'other' as const, mood: 'explore' as const, labels: ['Discover'], reasons: ['No stronger category signal was found, so VYBE keeps the place discoverable instead of hiding it.'] };
}
...
function analyzePlace(tags: Record<string, string>, name: string, rating = 0, reviews = 0): VybeAnalysis {
  ...
  if (hasPhoto) score += 4; if (hasHours) score += 3; if (hasAddress) score += 3;
...
function elementToPlace(element: any, lat: number, lng: number): VybePlace | null {
  ...
  const name = clean(tags.name || tags['name:fr'] || tags['name:ar']); if (!name) return null;
  const point = Number.isFinite(element?.lat) && Number.isFinite(element?.lon) ? { lat: Number(element.lat), lng: Number(element.lon) } : Number.isFinite(element?.center?.lat) && Number.isFinite(element?.center?.lon) ? { lat: Number(element.center.lat), lng: Number(element.center.lon) } : null;
...
async function searchOsm(options: DiscoverOptions): Promise<VybePlace[]> {
  ...
  const rawQuery = clean(options.query); const normalized = normalize(rawQuery);
  let clauses: string[];
  if (options.category && options.category !== 'all') {
    const category = options.category as Exclude<VybeCategory, 'other'>;
    clauses = CATEGORY_QUERIES[category] ?? BROAD_QUERIES;
  ...
  const endpoints = ['https://overpass-api.de/api/interpreter', 'https://overpass.kumi.systems/api/interpreter']; let lastError: unknown = null;
  for (const endpoint of endpoints) { try { const elements = await fetchOverpass(endpoint, query); const map = new Map<string, VybePlace>(); for (const element of elements) { const place = elementToPlace(element, options.lat, options.lng); if (place) map.set(place.id, place); } return [...map.values()].sort((a, b) => a.distanceKm - b.distanceKm).slice(0, SEARCH_LIMIT); } catch (error) { lastError = error; } }
...
export async function discoverEverything(options: DiscoverOptions): Promise<VybePlace[]> {
  ...
  const category = options.category; const query = normalize(clean(options.query));
  return results.filter(place => !category || category === 'all' || place.analysis.category === category || (category === 'gaming' && place.tags.some(tag => normalize(tag).includes('game')))).filter(place => !query || normalize(`${place.name} ${place.address} ${place.tags.join(' ')}`).includes(query)).sort((a, b) => (b.analysis.score - a.analysis.score) || (a.distanceKm - b.distanceKm));
}
...
export function getCityLabel(lat: number, lng: number): string {
  ...
  if (Math.abs(lat - 35.70) < 0.6 && Math.abs(lng + 0.63) < 0.6) return 'Oran';
