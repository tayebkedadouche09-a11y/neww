import type {
  ...
  PlacePhotoIdentity,
  VybeCategory,
  PlaceRelevanceEvidence,
} from '../types';
import type { GooglePlaceResult } from './googlePlacesTypes';
import {
...
function googlePhotoIdentities(
  ...
  return (photos ?? []).flatMap(photo => {
    ...
    for (const author of photo.author_attributions ?? []) {
      ...
      if (
        displayName &&
        !attributions.some(a => a.displayName === displayName && a.uri === author.uri)
      ) {
        attributions.push({
          displayName,
    ...
    return [
      {
        ...
        providerPlaceId,
        photoName: name || `photo-${providerPlaceId}`,
        uri,
        attributions,
      },
    ];
...
function googlePhotoAttributions(identities: PlacePhotoIdentity[]): PlacePhotoAttribution[] {
...
function hasLateNightHours(weekdayText?: string[]): boolean {
  return (weekdayText ?? []).some(entry => {
    ...
    return (
      /24\s*hours|open\s*24/.test(value) ||
      /(?:1|2|3|4|5)(?::\d{2})?\s*(?:am|a\.m\.)\b/.test(value)
    );
  });
}
...
export function isGooglePlaceValidForRequest(
  place: Place,
  request?: { query?: string; categories?: CategoryType[] }
): boolean {
  ...
  const canonicalFromQuery = raw ? normalizeCategoryQuery(raw) : null;
  const categoryTargets = canonicalFromQuery
    ? [canonicalFromQuery]
    : (request?.categories ?? []).flatMap(category => LEGACY_CATEGORY_TO_CANONICAL[category] ?? []);
  const requestedCategory = categoryTargets.length === 1 ? categoryTargets[0] : null;
  ...
  return (
    ...
    (!categoryTargets.length && evidence.identityValid && evidence.providerEvidenceSufficient)
  );
}
...
export function googlePlaceToVybePlace(gp: GooglePlaceResult): Place {
  ...
  const categoryDefinition: ProviderCategoryDefinition =
    VYBE_CATEGORY_DEFINITIONS[analysis.canonicalCategory];
  const photoIdentities = googlePhotoIdentities(gp.photos, providerPlaceId || '');
  const photoAttributions = googlePhotoAttributions(photoIdentities);
  ...
  const isFree =
    gp.price_level === 0 ||
    providerTypes.some(type =>
      ['park', 'playground', 'library', 'place_of_worship'].includes(type)
    );
  const canonicalCategory: VybeCategory = analysis.canonicalCategory;

  const place: Place = {
    id: providerPlaceId
      ? `google:${providerPlaceId}`
      : `google:${encodeURIComponent(gp.name || 'place')}`,
    ...
    providerIdentityConfidence: analysis.confidence,
    name: gp.name,
    ...
    baseVybeScore: Math.max(
      1,
      Math.min(99, Math.round(70 + analysis.confidence * 15 + Math.min(10, (gp.rating ?? 0) * 2)))
    ),
    images: photoIdentities.map(p => p.uri),
    photoAttributions,
    ...
    features: {
      isFree,
      isOutdoor: providerTypes.some(t =>
        [
          'park',
          'city_park',
          'state_park',
      ...
      hasFood: providerTypes.some(t =>
        ['restaurant', 'cafe', 'bakery', 'meal_takeaway', 'meal_delivery', 'bar', 'food_court'].includes(
          t
        )
      ),
      hasAlcohol: providerTypes.some(t =>
        ['bar', 'night_club', 'cocktail_bar', 'brewery', 'wine_bar'].includes(t)
      ),
  ...
  place.relevance = evidence as PlaceRelevanceEvidence;
  return place;
}
