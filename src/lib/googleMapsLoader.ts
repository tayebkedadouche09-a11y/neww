...
async function finalize(): Promise<{
  ...
  const classicMarker = (window.google!.maps as any).Marker ?? (markerLib as unknown as { Marker?: typeof google.maps.Marker }).Marker;
  if (typeof classicMarker !== 'function') {
    throw new Error('Google Maps Marker class is unavailable.');
  }

  return {
    Map: mapsLib.Map,
    AdvancedMarkerElement: markerLib.AdvancedMarkerElement,
    Marker: classicMarker,
  };
}

/**
 * Resolves once Maps + marker libraries are available. Both the modern
 * AdvancedMarkerElement and the classic Marker are returned so callers can
 * choose: advanced markers require a map ID and fail silently on maps without
 * one, while classic markers work everywhere.
...
export function loadGoogleMaps(): Promise<{
  ...
  if (isGoogleMapsLoaded()) {
    return Promise.resolve({
      ...
      Marker: (window.google!.maps as any).Marker ??
        ((window.google!.maps as any).marker as { Marker?: typeof google.maps.Marker }).Marker,
    });
  }
