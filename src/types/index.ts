...
/** Provider-neutral category used for discovery, matching and semantic classification. */
...
export interface PlaceReview {
  id: string; userId: string; userName: string; userAvatar: string; rating: number;
  vibeRating: number; moodTags: MoodType[]; comment: string; createdAt: string;
  images?: string[]; likesCount: number;
}

export interface PlaceOpeningHours {
  monday: string; tuesday: string; wednesday: string; thursday: string;
  friday: string; saturday: string; sunday: string; isOpenNow?: boolean;
}
...
export interface Place {
  ...
  primaryMood: MoodType; secondaryMoods: MoodType[];
  location: { address: string; neighborhood: string; city: string; lat: number; lng: number; };
  distanceKm?: number;
  priceLevel: PriceLevel; approxCostUsd: number; rating: number; reviewCount: number;
  baseVybeScore: number; images: string[];
  photoAttributions?: PlacePhotoAttribution[];
  providerTypes?: string[]; providerPrimaryType?: string;
  tags: string[]; estimatedDuration: string; openingHours: PlaceOpeningHours;
  features: {
    isFree: boolean; isOutdoor: boolean; isIndoor: boolean; hasFood: boolean; hasAlcohol: boolean;
    isLateNight: boolean; isSecretGem: boolean; isPetFriendly: boolean; isWifiFriendly: boolean;
    isPhotoSpot: boolean; isAccessible: boolean;
  };
  suitableFor: CompanionType[]; website?: string; phone?: string; instagram?: string;
  isFeatured?: boolean; isTrending?: boolean; reviews: PlaceReview[];
  provider?: ProviderType; providerPlaceId?: string;
}
